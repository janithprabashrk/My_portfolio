...
// Presets to quickly tweak the look
...
const Hyperspeed = ({ effectOptions = hyperspeedPresets.one, className }: HyperspeedProps) => {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const appRef = useRef<any>(null);
  const ioRef = useRef<IntersectionObserver | null>(null);

  useEffect(() => {
    // Clean previous instance (hot reloads, prop changes)
    if (appRef.current) {
    ...
    // --- Distortion uniforms and helpers ---
    ...
    const distortions: any = {
      ...
      xyDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          const movementProgressFix = 0.02;
          const uFreq = xyUniforms.uFreq.value as any;
          const uAmp = xyUniforms.uAmp.value as any;
          const distortion = new THREE.Vector3(
            Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x -
              Math.cos(movementProgressFix * Math.PI * uFreq.x + time) * uAmp.x,
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          ...
          const uFreq = LongRaceUniforms.uFreq.value as any;
          const uAmp = LongRaceUniforms.uAmp.value as any;
          const distortion = new THREE.Vector3(
      ...
      turbulentDistortion: {
        ...
        `,
        getJS: (progress: number, time: number) => {
          const uFreq = turbulentUniforms.uFreq.value as any;
          const uAmp = turbulentUniforms.uAmp.value as any;
          const getX = (p: number) =>
      ...
      deepDistortion: {
        ...
        `,
        getJS: (progress: number, time: number) => {
          const uFreq = deepUniforms.uFreq.value as any;
          const uAmp = deepUniforms.uAmp.value as any;
          const uPowY = deepUniforms.uPowY.value as any;
  ...
  const dpr = Math.min(0.85, window.devicePixelRatio || 1);
        ...
        // bind handlers once for add/remove
  ...
  const smaaPass = new EffectPass(this.camera, new SMAAEffect({ preset: SMAAPreset.MEDIUM }));
      ...
      loadAssets() {
        // SMAAEffect in postprocessing has built-in images; no external asset loading needed
  ...
  this.rafId = 0;
        ...
        if (this.container) {
          ...
          this.container.removeEventListener("contextmenu", this.onContextMenu as any);
        }
      ...
      tick() {
        ...
        if (!this.active) {
          // keep rAF stopped while inactive
          if (this.rafId) cancelAnimationFrame(this.rafId);
          this.rafId = 0;
          return;
        }
        if (resizeRendererToDisplaySize(this.renderer, this.setSize)) {
          ...
          this.camera.aspect = Math.max(canvas.clientWidth, 1) / Math.max(canvas.clientHeight, 1);
          this.camera.updateProjectionMatrix();
        }
    ...
    class CarLights {
      ...
      init() {
        ...
        const instanced = new (THREE as any).InstancedBufferGeometry().copy(geometry);
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          aColor.push((color as any).r, (color as any).g, (color as any).b);
          aColor.push((color as any).r, (color as any).g, (color as any).b);
        }
        instanced.setAttribute("aOffset", new (THREE as any).InstancedBufferAttribute(new Float32Array(aOffset), 3, false));
        instanced.setAttribute("aMetrics", new (THREE as any).InstancedBufferAttribute(new Float32Array(aMetrics), 3, false));
        instanced.setAttribute("aColor", new (THREE as any).InstancedBufferAttribute(new Float32Array(aColor), 3, false));
        ...
        const mesh = new THREE.Mesh(instanced as any, material);
    ...
    class LightsSticks {
      ...
      init() {
        ...
        instanced.setAttribute("aOffset", new (THREE as any).InstancedBufferAttribute(new Float32Array(aOffset), 1, false));
        instanced.setAttribute("aColor", new (THREE as any).InstancedBufferAttribute(new Float32Array(aColor), 3, false));
        instanced.setAttribute("aMetrics", new (THREE as any).InstancedBufferAttribute(new Float32Array(aMetrics), 2, false));
        const material = new THREE.ShaderMaterial({
    ...
    (function () {
      ...
      // Auto-pause when out of view, tab hidden, or user prefers reduced motion
      const prefersReducedMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
      if (prefersReducedMotion) myApp.setActive(false);
      ...
      ioRef.current = new IntersectionObserver((entries) => {
        ...
        myApp.setActive(visible && !document.hidden && !prefersReducedMotion);
      }, { root: null, threshold: 0.1 });
      ioRef.current.observe(container);

      const onVis = () => myApp.setActive(!document.hidden && (ioRef.current ? true : true));
      document.addEventListener("visibilitychange", onVis);
      // Cleanup IO and listeners on unmount
      (myApp as any)._cleanup = () => {
  ...
  return <div ref={containerRef} className={["absolute inset-0", "pointer-events-none", className].filter(Boolean).join(" ")} />;
};
